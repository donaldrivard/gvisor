...
// lockFieldFacts apply on every struct field protected by a lock.
...
type lockFunctionFacts struct {
	...
	// fields that guard calls to this function.
...
var (
	mutexRE   = regexp.MustCompile("((.*/)|^)sync.Mutex")
	rwMutexRE = regexp.MustCompile("((.*/)|^)sync.RWMutex")
)
...
func (pc *passContext) findAndExportStructFacts(ss *ast.StructType, structType *types.Struct) {
	...
	// Only export facts if the structure contains any annotations. We do not support
	// checking mutexes in structures which do not carry any annotations. Without this
	// check the analyzer ends up analyzing non annotated code and fails.
...
outerLoop:
	for _, l := range d.Doc.List {
		...
		if strings.HasPrefix(l.Text, "// +checklocks:") {
			...
			for paramNum, field := range paramList {
				...
				if len(field.Names) == 0 || field.Names[0].Name != paramName {
					log.Debugf("skipping because parameter is either unnamed or name does not match parameter name %s specified in annotation", paramName)
					continue
				}
	...
	if ignore {
		pc.pass.Reportf(ignorePos, "//+checklocks:ignore cannot be specified with other annotations on the function")
	}
...
func (pc *passContext) checkFunctionCall(call *ssa.Call, isExempted bool, lh *locksHeld) {
	...
	if fn.Object() == nil {
		log.Debugf("fn w/ nil Object is: %+v", fn)
		return
	}
